...
class PersonDataConverter:
    ...
    def __create_tables(self):
        ...
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personName
                     (id integer PRIMARY KEY,
                      title text,
                      first text,
                      last text
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personLocation
                     (id integer PRIMARY KEY,
                      city text,
                      state text,
                      country text,
        ...
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personLocationStreet
                     (
                      id integer PRIMARY KEY,
                      number text,
                      name text
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personLocationCoordinates
                     (
                      id integer PRIMARY KEY,
                      latitude text,
                      longitude text
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personLocationTimezone
                     (
                      id integer PRIMARY KEY,
                      offset text,
                      description text
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personLogin
                     (
                      id integer PRIMARY KEY,
                      uuid text,
                      username text,
        ...
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personDOB
                     (
                      id integer PRIMARY KEY,
                      date text,
                      age integer
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personRegistered
                     (
                      id integer PRIMARY KEY,
                      date text,
                      age integer
                     )''')
        self.__cur.execute('''CREATE TABLE IF NOT EXISTS personId
                     (
                      id integer PRIMARY KEY,
                      name text,
                      value text
    ...
    def __days_to_birthday(self, month: int, day: int) -> int:
        ...
        if bd_this_year >= -1:
            return bd_this_year + 1  # birthday is today or this year
        ...
        return bd_next_year + 1  # birthday is  next year
